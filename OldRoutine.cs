...
namespace OldRoutine
{
	...
	public class OldRoutine : IRoutine
	{
        ...
        private int _convovationSlot = -1;
        private int _arcSlot = -1;
        ...
        private readonly Stopwatch _surroundStopwatch = Stopwatch.StartNew();
		...
		public void Tick()
		{
			...
			if (_needsUpdate)
			{
				...
				foreach (var skill in LokiPoe.InGameState.SkillBarPanel.Skills)
				{
					...
					if (_auraSlot == -1 && ((tags.Contains("aura") && !tags.Contains("vaal") && !skill.IsTotem) || IsAuraName(name)))
					{
                ...
                var des = LokiPoe.InGameState.SkillBarPanel.Skills.FirstOrDefault(s => s.Name == "Desecrate");
                if (IsCastableHelper(des))
                {
                    _desecrateSlot = des.Slot;
                }

                var con = LokiPoe.InGameState.SkillBarPanel.Skills.FirstOrDefault(s => s.Name == "Convocation");
                if (IsCastableHelper(con))
                {
                    _convovationSlot = con.Slot;
                }

                var fof = LokiPoe.InGameState.SkillBarPanel.Skills.FirstOrDefault(s => s.Name == "Flesh Offering");
                if (IsCastableHelper(fof))
                {
                    _fleshofferingSlot = fof.Slot;
                }
				...
				var arc = LokiPoe.InGameState.SkillBarPanel.Skills.FirstOrDefault(s => s.Name == "Arc");
				if (IsCastableHelper(arc))
				{
					_arcSlot = arc.Slot;
				}
		...
		public async Task<bool> Logic(string type, params dynamic[] param)
		{
			...
			if (type == "combat")
			{
                ...
                //Listing our minions
                var Spectres = LokiPoe.InGameState.SkillBarPanel.Skills.FirstOrDefault(s => s.Name == "Raise Spectre").DeployedObjects;
                var Zombies = LokiPoe.InGameState.SkillBarPanel.Skills.FirstOrDefault(s => s.Name == "Raise Zombie").DeployedObjects;
                var VaalSkeletons = LokiPoe.InGameState.SkillBarPanel.Skills.FirstOrDefault(s => s.Name == "Vaal Summon Skeletons");
                var VaalDiscipline = LokiPoe.InGameState.SkillBarPanel.Skills.FirstOrDefault(s => s.Name == "Vaal Discipline");
                var VaalHaste = LokiPoe.InGameState.SkillBarPanel.Skills.FirstOrDefault(s => s.Name == "Vaal Haste");
				...
				if (_raiseZombieSlot != -1)
				{
					...
					if (skill.CanUse())
					{
						...
						if (skill.NumberDeployed < max)
						{
                            ...
                            else if (_desecrateSlot != -1)
                            {
                                var des = LokiPoe.InGameState.SkillBarPanel.Slot(_desecrateSlot);
                                if (des.CanUse())
                                {
                                    var uaerr = LokiPoe.InGameState.SkillBarPanel.UseAt(_desecrateSlot, false, myPos);
                                    if (uaerr == LokiPoe.InGameState.UseError.None)
                                    {
                                        await Coroutines.FinishCurrentAction(false);
                ...
                //Keeping Spectres alive
                if (_convocationStopwatch.ElapsedMilliseconds > 3000)
                {
                    _convocationStopwatch.Restart();
                    foreach (NetworkObject n in Spectres)
                    {
                        Monster m = (Monster)n;
                        if (m.HealthPercent <= 50)
                        {
                            foreach (Item f in LokiPoe.InGameState.QuickFlaskPanel.Flasks)
                            {
                                Loki.Game.Objects.Items.Flask flask = (Loki.Game.Objects.Items.Flask)f;
                                if (flask.ExplicitAffixes.Any(a => a.DisplayName == "LocalFlaskMinionHealPct") && flask.CurrentCharges >= flask.ChargesPerUse)
                                {                                 
                                    LokiPoe.InGameState.QuickFlaskPanel.UseFlask(f);
                                    await Coroutines.FinishCurrentAction(false);
                                    await Coroutine.Sleep(Utility.LatencySafeValue(100));
                                    return true;
                                }
                            }
                            if (_convovationSlot != -1)
                            {
                                var skill = LokiPoe.InGameState.SkillBarPanel.Slot(_convovationSlot);
                                if (skill.CanUse())
                                {
                                    var err1 = LokiPoe.InGameState.SkillBarPanel.Use(_convovationSlot, true);
                ...
                var blockedByDoor = Utility.ClosedDoorBetween(LokiPoe.Me, bestTarget, 10, 10, !OldRoutineSettings.Instance.LeaveFrame);
                ...
                //Handle surrounding
                if ((this.NumberMagicMobsMiddleRange >= 3 || this.NumberNormalMobsCloseRange >= 3 || this.NumberNormalMobsMiddleRange >= 9 || this.NumberRareUniqueMobsMiddleRange >= 1) && _surroundStopwatch.ElapsedMilliseconds >= 8000)
                {
                    if (_convovationSlot != -1)
                    {
                        var skill = LokiPoe.InGameState.SkillBarPanel.Slot(_convovationSlot);
                ...
                //Handle flesh offering
                if (_fleshofferingSlot != -1)
                {
                    var fof = LokiPoe.InGameState.SkillBarPanel.Slot(_fleshofferingSlot);
                    if (fof.CanUse() && !(((Monster)Zombies.FirstOrDefault()).HasFleshOffering))
                    {
                        var corpse = BestDeadTarget;                       
                        if (corpse != null)
                        {
                            var uaerr = LokiPoe.InGameState.SkillBarPanel.UseAt(_fleshofferingSlot, false, corpse.Position);
                            if (uaerr == LokiPoe.InGameState.UseError.None)
                            {
                ...
                //Vaal Skeletons to fight bosses
                if (IsCastableHelper(VaalSkeletons))
                {
                    if (bestTarget.Rarity == Rarity.Unique && bestTarget.Name != "Summoned Skeleton General" && VaalSkeletons.CurrentSouls >= VaalSkeletons.SoulsPerUse)
                    {
                        if (VaalSkeletons.Slot == -1)
                        {
                            LokiPoe.InGameState.SkillBarPanel.SetSlot8(VaalSkeletons);
                            await Coroutines.FinishCurrentAction(false);
                            await Coroutine.Sleep(Utility.LatencySafeValue(100));
                        }

                        LokiPoe.InGameState.SkillBarPanel.UseAt(VaalSkeletons.Slot, false, cachedPosition);
                        await Coroutines.FinishCurrentAction(false);
                        await Coroutine.Sleep(Utility.LatencySafeValue(100));
                        return true;
                    }
                }
                //Vaal Haste to fight bosses
                if (IsCastableHelper(VaalHaste))
                {
                    if (bestTarget.Rarity == Rarity.Unique && bestTarget.Name != "Summoned Skeleton General" && VaalHaste.CurrentSouls >= VaalHaste.SoulsPerUse)
                    {
                        if (VaalHaste.Slot == -1)
                        {
                            LokiPoe.InGameState.SkillBarPanel.SetSlot8(VaalHaste);
                            await Coroutines.FinishCurrentAction(false);
                            await Coroutine.Sleep(Utility.LatencySafeValue(100));
                        }

                        LokiPoe.InGameState.SkillBarPanel.UseAt(VaalHaste.Slot, false, cachedPosition);
                        await Coroutines.FinishCurrentAction(false);
                        await Coroutine.Sleep(Utility.LatencySafeValue(100));
                        return true;
                    }
                }
                //Vaal Discipline if ES goes down
                if (IsCastableHelper(VaalDiscipline))
                {
                    if (LokiPoe.Me.EnergyShieldPercent <= 50 && VaalDiscipline.CurrentSouls >= VaalDiscipline.SoulsPerUse)
                    {
                        if (VaalDiscipline.Slot == -1)
                        {
                            LokiPoe.InGameState.SkillBarPanel.SetSlot8(VaalDiscipline);
                            await Coroutines.FinishCurrentAction(false);
                            await Coroutine.Sleep(Utility.LatencySafeValue(100));
                        }

                        LokiPoe.InGameState.SkillBarPanel.UseAt(VaalDiscipline.Slot, false, cachedPosition);
                        await Coroutines.FinishCurrentAction(false);
                        await Coroutine.Sleep(Utility.LatencySafeValue(100));
                        return true;
                    }
                ...
                if (_arcSlot != -1)
                {
                    ...
                    var err = LokiPoe.InGameState.SkillBarPanel.UseAt(_arcSlot, false, targetPosition);
		...
		public object Execute(string name, params dynamic[] param)
		{
			...
			if (name == "SetCombatRange")
			{
				OldRoutineSettings.Instance.CombatRange = (int)param[0];
			}
		...
		private Monster BestDeadTarget
		{
			get
			{
				...
				return LokiPoe.ObjectManager.GetObjectsByType<Monster>()
					.Where(
						m =>
							m.Distance < 75 && m.IsActiveDead && m.Rarity != Rarity.Unique && m.CorpseUsable &&
                            ExilePather.PathDistance(myPos, m.Position, true, !OldRoutineSettings.Instance.LeaveFrame) < 75)
					.OrderBy(m => m.Distance).FirstOrDefault();
			}
		}


        private int NumberOfCorpsesNearby
        {
            get
            {
                var myPos = LokiPoe.Me.Position;
                return LokiPoe.ObjectManager.GetObjectsByType<Monster>()
                    .Where(
                        m =>
                            ...
                            ExilePather.PathDistance(myPos, m.Position, true, !OldRoutineSettings.Instance.LeaveFrame) < 30).Count();
                   
            }
        }

        private int NumberNormalMobsMiddleRange
        {
            get
            {
                var myPos = LokiPoe.Me.Position;
                return LokiPoe.ObjectManager.GetObjectsByType<Monster>()
                    .Where(
                        m =>
                            m.Distance < 30 && !m.IsDead && m.Rarity == Rarity.Normal && m.IsHostile &&
                            ExilePather.PathDistance(myPos, m.Position, true, !OldRoutineSettings.Instance.LeaveFrame) < 30).Count();
                   
            }
        }

        private int NumberNormalMobsCloseRange
        {
            get
            {
                var myPos = LokiPoe.Me.Position;
                return LokiPoe.ObjectManager.GetObjectsByType<Monster>()
                    .Where(
                        m =>
                            m.Distance < 15 && !m.IsDead && m.Rarity == Rarity.Normal && m.IsHostile &&
                            ExilePather.PathDistance(myPos, m.Position, true, !OldRoutineSettings.Instance.LeaveFrame) < 15).Count();
            }
        }

        private int NumberMagicMobsMiddleRange
        {
            get
            {
                var myPos = LokiPoe.Me.Position;
                return LokiPoe.ObjectManager.GetObjectsByType<Monster>()
                    .Where(
                        m =>
                            m.Distance < 30 && !m.IsDead && m.Rarity == Rarity.Magic && m.IsHostile &&
                            ExilePather.PathDistance(myPos, m.Position, true, !OldRoutineSettings.Instance.LeaveFrame) < 30).Count();

            }
        }

        private int NumberRareUniqueMobsMiddleRange
        {
            get
            {
                var myPos = LokiPoe.Me.Position;
                return LokiPoe.ObjectManager.GetObjectsByType<Monster>()
                    .Where(
                        m =>
                            m.Distance < 30 && !m.IsDead && (m.Rarity == Rarity.Rare || m.Rarity == Rarity.Unique) && m.IsHostile &&
                            ExilePather.PathDistance(myPos, m.Position, true, !OldRoutineSettings.Instance.LeaveFrame) < 30).Count();
